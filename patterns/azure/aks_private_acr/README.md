# Azure AKS Private ACR Pattern

This pattern composes a private Azure Kubernetes Service cluster, a private Azure Container Registry, Azure Private Link for ACR, a small self-contained VNet, Azure Bastion, NAT Gateway egress, subnet-level NSG rules, and a lightweight private jump host into a public teaser-tier AKS landing-zone pattern.

It builds on `aks_basic` by adding ACR Private Endpoint access. It is still intentionally narrower than the premium `aks_firewall_transit` blueprint. Upgrade to that blueprint when the architecture requires Firewall-based transit, hub-and-spoke networking, customer-managed keys, multiple node pools, or broader platform governance.

## Summary

This pattern is intentionally scoped to a **single-region private AKS plus private ACR baseline**:

- one VNet with one AKS node subnet, one jump subnet, one ACR Private Endpoint subnet, and `AzureBastionSubnet`
...
- one Private DNS Zone named `privatelink.azurecr.io` linked to the VNet
- one Azure Bastion host used for operator SSH access to the private jump host
- one lightweight Linux jump host used to validate private AKS API and ACR TCP `443` reachability
...
- image build, push, signing, scanning, or promotion workflows
...
Those concerns belong in the premium `aks_firewall_transit` blueprint or later public patterns rather than this focused public teaser-tier pattern.
...
Optional `data.aks.cmk`, `data.aks.diagnostics`, and `data.aks.additional_node_pools` blocks are decoded defensively but rejected by this pattern until a richer blueprint-tier implementation wires them explicitly.

## Verified Service Names

- ACR Private Endpoint subresource: `registry`
