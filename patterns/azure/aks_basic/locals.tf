locals {
  ...
  project_name = nonsensitive(try(local.landing_zone.name, "fk-azure-aks-basic-dev"))
  ...
  aks_acr             = nonsensitive(try(local.aks.acr, null))
  ...
  default_jump_custom_data = <<-EOT
    ...
    write_files:
      - path: /opt/fk-aks-basic/README.txt
        ...
        permissions: "0644"
        content: |
          FoggyKitchen Azure AKS Basic validation host
          -----------------------------------------------------------
          Use this host to validate private AKS API reachability.
